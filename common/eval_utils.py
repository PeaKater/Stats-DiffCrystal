...
from stats_diffcrystal.predict_property import CrystGNN_Supervise
...
from common.dataset import TensorCrystDataset
from common.datamodule import CrystDataModule, worker_init_fn
...
def get_model_path(eval_model_name):
    import stats_diffcrystal
    model_path = (
        Path(stats_diffcrystal.__file__).parent / 'prop_models' / eval_model_name)
    return model_path

# 加载模型配置
def load_config(model_path):
    cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/'hparams.yaml')), resolve=True))
    # with initialize_config_dir(str(model_path)):
    #     cfg = compose(config_name='hparams')
    return cfg

# 加载模型
def load_model(MODEL, model_path, load_data=False, testing=True, prop_model=False, ss=True):
    cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/'hparams.yaml')), resolve=True))
    hparams = cfg.model
    ...
    if not prop_model:
        ...
        model.init(sample=True)
    else:        
        cfg.output_dir = str(model_path)
        cfg.data.datamodule._target_=''
        model = MODEL(cfg).to(device)
        ckpts = list(model_path.glob(f'*={model.model_name}=*.ckpt'))
        if len(ckpts) > 0:
    ...
    if load_data:
        datamodule = CrystDataModule(**{k:v for k, v in cfg.data.datamodule.items() if k!='_target_'})
        if testing:
...
# 加载控制模型
def load_control(MODEL, model_path, num_classes):
    ...
    hparams.data = cfg.data
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = MODEL(hparams, num_classes).to(device)
    model.device = device

    ckpts = list(model_path.glob('*=control=*.ckpt'))
    if len(ckpts) > 0:
        ckpt_epochs = np.array(
            [int(ckpt.parts[-1].split('.')[0].split('=')[1]) for ckpt in ckpts])
        ckpt = str(ckpts[ckpt_epochs.argsort()[-1]])
...
def prop_model_eval(eval_model_name, crystal_array_list):
    ...
    #cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/'hparams.yaml')), resolve=True))
    model, _, cfg = load_model(CrystGNN_Supervise, model_path, prop_model=True)
    # or
    model.lattice_scaler = torch.load(cfg.output_dir  + '/lattice_scaler.pt')
    model.scaler = torch.load(cfg.output_dir  + '/prop_scaler.pt')[0]
...
# 加载分类器模型
def load_classifier(MODEL, model_path, num_classes):
    ...
    ckpts = list(model_path.glob('*=classifier=*.ckpt'))
    if len(ckpts) > 0:
        ckpt_epochs = np.array(
            [int(ckpt.parts[-1].split('.')[0].split('=')[1]) for ckpt in ckpts])
        ckpt = str(ckpts[ckpt_epochs.argsort()[-1]])
...
# 评估分类模型的性能
def class_model_eval(MODEL, MODEL_CLASSIFIER, model_path, crystal_array_list, num_classes):

    model, _, _ = load_model(MODEL, model_path)
    model_classifier = load_classifier(MODEL_CLASSIFIER, model_path, num_classes)
    cfg = load_config(model_path)
    ...
    for batch in loader:
        _, _, z = model.encode(batch.to(model.device))
        preds = model_classifier(z)
        preds = torch.nn.functional.softmax(preds, -1).argmax(-1)
