...
class BesselBasisLayer(torch.nn.Module):
    def __init__(self, num_radial=6, cutoff=5.0, envelope_exponent=5):
        ...
        freq = torch.arange(1, num_radial+1, dtype=torch.float32).mul_(PI)
        self.freq = freq.to('cuda')
...
# 定义了三个编码器模型DimeNetppEncoder、DimeNaEncoder和DimeGINNaEncoder
...
class DimeNetppEncoder(DimeNetPlusPlus):
    def __init__(
        self,
        num_targets,
        hidden_channels=128,
    ...
    def forward(self, data):
        ...
        rbf = self.rbf(dist)
        if torch.any(torch.isnan(rbf)):
            raise ValueError("rbf error")
